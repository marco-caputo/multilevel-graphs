...
class MultilevelGraphTest(unittest.TestCase):
    ...
    def test_natural_transformation_2(self):
        nxGraph = nx.DiGraph()
        nxGraph.add_node(1, weight=1)
        nxGraph.add_node(2, weight=2)
        nxGraph.add_node(3, weight=3)
        nxGraph.add_edges_from([(1, 2, {'weight': 3}), (1, 3, {'weight': 4})])

        dec_graph = MultilevelGraph.natural_transformation(nxGraph)
        self.assertTrue(isinstance(dec_graph, DecGraph))
